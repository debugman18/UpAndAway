...
local function ondeploy(inst, pt) 
    ...
    local blob = SpawnPrefab("spiderden") 
    if blob then 
        blob.components:SetLeader(inst.components.inventoryitem.owner)
        blob.Transform:SetPosition(pt.x, pt.y, pt.z) 
...
local function fn(Sim)
    ...
    inst:AddComponent("tradeable")

    local deployable = inst.components.deployable or inst:AddComponent("deployable")

    deployable.test = function() return true end
    deployable.ondeploy = OnDeploy

    return inst
end

return Prefab ("common/inventory/cloud_egg", fn, assets) 